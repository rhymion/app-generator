...
export default function FormUpsert({ src, isEdit, permissions, initialApprovalFlows = [], initialRoles = [], searchApprovalFlowOptions, searchRoleOptions }: FormUpsertProps) {
  ...
  const entityNameOptions = [{ value: 'user', label: 'User' }, { value: 'setting', label: 'Setting' }, { value: 'role', label: 'Role' }, { value: 'organization', label: 'Organization' }, { value: 'permission', label: 'Permission' }, { value: 'approval_flow', label: 'Approval Flow' }];
  ...
  const formFields = (
    <>
      <Autocomplete
        ...
        onChange={(_, newValue) => setEntityName(newValue?.value ?? null)}
        renderInput={(params) => (
          <TextField
            {...params}
            label={tf('entityName')}
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
        ...
        {requestorRoleId && (
          <Tooltip title="View">
            <Link href={`/role/view/${requestorRoleId}`} aria-label="View Role">
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
        ...
        {approverRoleId && (
          <Tooltip title="View">
            <Link href={`/role/view/${approverRoleId}`} aria-label="View Role">
              <IconButton component="span" size="small" tabIndex={-1} sx={{ mt: 2 }}>
                <OpenInNewIcon fontSize="small" />
              </IconButton>
            </Link>
