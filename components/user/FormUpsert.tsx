...
export default function FormUpsert({ src, isEdit, permissions, initialRoles = [], searchRoleOptions }: FormUpsertProps) {
  ...
  const subAccountsRef = useRef<{ getItems: () => EditableListWrapperItem[] }>(null);
  ...
  const [localInitialSubAccounts] = useState<EditableListWrapperItem[]>(() => src.sub_accounts.map(f => ({
    id: f.id || `temp-${Date.now()}-${Math.random()}`,
    value: (f.nickname ?? ''),
    label: (f.nickname ?? ''),
    originalId: f.id,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const subAccounts = subAccountsRef.current?.getItems?.() || [];
  ...
  const formFields = (
    <>
      ...
      <EditableListWrapper
        ref={subAccountsRef}
        initialItems={localInitialSubAccounts}
        itemType="text"
        addButtonLabel="Add Sub Accounts"
        showTitle={true}
