import type { GridColDef } from '@/components/ui/data';
import { useTranslations } from 'next-intl';

export function useUsersColumns(editable: boolean = false): GridColDef[] {
  const t = useTranslations('Fields');
  return [
    { field: 'name', headerName: t('name'), width: 150, editable: editable },
    { field: 'email', headerName: t('email'), width: 150, editable: editable },
    { field: 'password', headerName: t('password'), width: 150, editable: editable },
    { field: 'api_key', headerName: t('apiKey'), width: 150, editable: editable },
    { field: 'image', headerName: t('image'), width: 150, editable: editable },
    { field: 'mfa_enabled', headerName: t('mfaEnabled'), width: 100, editable: editable, type: 'boolean' },
  ];
}

