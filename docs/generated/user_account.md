...
| Page | URL | Enabled |
|------|-----|:-------:|
| List | `/en/user_account` | ✓ |
| New | `/en/user_account/new` | ✗ |
| Edit | `/en/user_account/edit/[id]` | ✓ |
| View | `/en/user_account/view/[id]` | ✓ |
...
List all `User Account` records the authenticated user has permission to read.
...
Retrieve a single `User Account` record by its ID.
...
### DELETE /api/user_account/[id]

Delete a `User Account` record by its ID.
...
### PUT /api/user_account/bulk

Bulk-update `User Account` records. Each element must include `id` in addition to the
fields accepted by `PUT /api/user_account/[id]`.
...
### DELETE /api/user_account/bulk

Bulk-delete `User Account` records by ID.
