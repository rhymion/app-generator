...
### POST /api/db_table/bulk

Bulk-create `Db Table` records. Each element in the request array is processed with
the same rules as `POST /api/db_table`.
...
### PUT /api/db_table/bulk

Bulk-update `Db Table` records. Each element must include `id` in addition to the
fields accepted by `PUT /api/db_table/[id]`.
...
### DELETE /api/db_table/bulk

Bulk-delete `Db Table` records by ID.
