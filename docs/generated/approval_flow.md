# Approval Flow

> Auto-generated — run the code generator to update this file. Do not edit manually.

## Overview

| | |
|---|---|
| Entity | `approval_flow` |
| Operations | List · View · Create · Update · Delete |
| API | Yes — see [API Reference](#api-reference) below |

## Fields

| Field | Type | Required | Read-only | Notes |
|-------|------|:--------:|:---------:|-------|
| `id` | `string` | ✓ | ✓ | CUID |
| `entity_name` | `string` | ✓ |  | min 1 chars |
| `requestor_role_id` | `string \| null` |  |  | FK → [role](role.md) (label: `name`), CUID |
| `approver_role_id` | `string` | ✓ |  | FK → [role](role.md) (label: `name`), CUID |

> **Read-only** fields (`id`, `creator_id`, `created_at`, `updated_at`) are set automatically and cannot be written through the API or UI forms.

## Relationships

| Field / Property | Target Entity | Relationship Type |
|-----------------|---------------|-------------------|
| `requestor_role_id` | [`role`](role.md) | many-to-one |
| `approver_role_id` | [`role`](role.md) | many-to-one |
| `preceded_by` | [`approval_flow`](approval_flow.md) | many-to-many · `list` |
| `followed_by` | [`approval_flow`](approval_flow.md) | many-to-many · `list` |

## UI Pages

| Page | URL | Enabled |
|------|-----|:-------:|
| List | `/en/approval_flow` | ✓ |
| New | `/en/approval_flow/new` | ✓ |
| Edit | `/en/approval_flow/edit/[id]` | ✓ |
| View | `/en/approval_flow/view/[id]` | ✓ |

## API Reference

### Authentication

All endpoints require one of:

| Header | Format | Example |
|--------|--------|---------|
| `X-API-Key` | `<key>` | `X-API-Key: mk_abc123...` |
| `Authorization` | `Bearer <key>` | `Authorization: Bearer mk_abc123...` |

API keys are stored in `user.api_key` and start with the `mk_` prefix.

---

### GET /api/approval_flow

List all `Approval Flow` records the authenticated user has permission to read.

**Response `200 OK`**

```json
[
  {
    "id": "clxxxxxxxxxxxxxxxxxxxxxxxx",
    "entity_name": "...",
    "requestor_role_id": null,
    "approver_role_id": "..."
  }
]
```

### POST /api/approval_flow

Create a new `Approval Flow` record.

**Request Body**

```json
{
  "entity_name": "...",
  "requestor_role_id": null,
  "approver_role_id": "...",
  "precededBy_ids": ["clxxx..."],
  "followedBy_ids": ["clxxx..."]
}
```

**Child / association fields**

| Field | Type | Description |
|-------|------|-------------|
| `precededBy_ids` | `string[]` | IDs of `approval_flow` records to associate (replaces existing) |
| `followedBy_ids` | `string[]` | IDs of `approval_flow` records to associate (replaces existing) |

**Response `201 Created`** — the newly created record object.

### GET /api/approval_flow/[id]

Retrieve a single `Approval Flow` record by its ID.

**Response `200 OK`** — the record object (same shape as a list item).

**Response `404 Not Found`**

```json
{ "error": "Not found" }
```

### PUT /api/approval_flow/[id]

Replace an existing `Approval Flow` record. 
Accepts the same body as `POST /api/approval_flow`.

**Response `200 OK`**

```json
{ "success": true }
```

**Response `404 Not Found`**

```json
{ "error": "Not found" }
```

### DELETE /api/approval_flow/[id]

Delete a `Approval Flow` record by its ID.

**Response `204 No Content`** — empty body on success.

**Response `404 Not Found`**

```json
{ "error": "Not found" }
```


### Error Responses

| Status | Meaning |
|--------|---------|
| `400` | Bad request (validation error) |
| `401` | Missing or invalid API key |
| `403` | Insufficient permission for the operation |
| `404` | Record not found |
| `500` | Unexpected server error |

All error responses return `{ "error": "<message>" }`.

---

## Bulk Operations

Bulk endpoints process each item independently, enabling **partial success**. The outer
request always returns `207 Multi-Status` as long as authentication passes; individual
item outcomes are reported in the `results` array.

### Response shape (`207 Multi-Status`)

```json
{
  "results": [
    { "index": 0, "success": true,  "data": { ... } },
    { "index": 1, "success": false, "error": "Not found: clxxx..." }
  ],
  "summary": { "total": 2, "succeeded": 1, "failed": 1 }
}
```

> Auth/permission failures at the **request** level (bad key, no permission at all) still
> return `401` / `403` — not `207`.

### POST /api/approval_flow/bulk

Bulk-create `Approval Flow` records. Each element in the request array is processed with
the same rules as `POST /api/approval_flow`.

**Request Body** — array of create objects:

```json
[
  {
    "entity_name": "...",
    "requestor_role_id": null,
    "approver_role_id": "...",
    "precededBy_ids": ["clxxx..."],
    "followedBy_ids": ["clxxx..."]
  },
  { "..." : "..." }
]
```

**Response `207`** — `data` of each successful item contains `{ "id": "..." }`.

### PUT /api/approval_flow/bulk

Bulk-update `Approval Flow` records. Each element must include `id` in addition to the
fields accepted by `PUT /api/approval_flow/[id]`.

**Request Body** — array of update objects:

```json
[
  { "id": "clxxx...", 
    "entity_name": "...",
    "requestor_role_id": null,
    "approver_role_id": "...",
    "precededBy_ids": ["clxxx..."],
    "followedBy_ids": ["clxxx..."]
  },
  { "id": "clyyy...", "..." : "..." }
]
```

**Response `207`** — `data` of each successful item is `{ "success": true }`.
Items with an unknown `id` report `"error": "Not found: <id>"`.

### DELETE /api/approval_flow/bulk

Bulk-delete `Approval Flow` records by ID.

**Request Body** — array of id objects:

```json
[
  { "id": "clxxx..." },
  { "id": "clyyy..." }
]
```

**Response `207`** — `data` of each successful item is `null`.
Items with an unknown `id` report `"error": "Not found: <id>"`.

