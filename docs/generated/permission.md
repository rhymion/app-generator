...
### POST /api/permission/bulk

Bulk-create `Permission` records. Each element in the request array is processed with
the same rules as `POST /api/permission`.
...
### PUT /api/permission/bulk

Bulk-update `Permission` records. Each element must include `id` in addition to the
fields accepted by `PUT /api/permission/[id]`.
...
### DELETE /api/permission/bulk

Bulk-delete `Permission` records by ID.
