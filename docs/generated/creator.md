# Creator

> Auto-generated — run the code generator to update this file. Do not edit manually.

## Overview

| | |
|---|---|
| Entity | `creator` |
| Operations | List · View · Create · Update · Delete |
| API | Yes — see [API Reference](#api-reference) below |

## Fields

| Field | Type | Required | Read-only | Notes |
|-------|------|:--------:|:---------:|-------|
| `id` | `string` | ✓ | ✓ | CUID |
| `name` | `string` | ✓ |  | min 1 chars |
| `role` | `integer` | ✓ |  | min: 0, max: 3, values: `voice`, `anim`, `bgm`, `etc` |
| `affiliation` | `integer` | ✓ |  | min: 0, max: 2, values: `agency`, `freelance`, `student` |

> **Read-only** fields (`id`, `creator_id`, `created_at`, `updated_at`) are set automatically and cannot be written through the API or UI forms.

## Relationships

| Field / Property | Target Entity | Relationship Type |
|-----------------|---------------|-------------------|
| `voiced_characters` | [`character`](character.md) | many-to-many · `list` |
| `composed_musics` | [`music`](music.md) | many-to-many · `list` |
| `credited_musics` | [`music`](music.md) | many-to-many · `list` |
| `credited_scenes` | [`scene`](scene.md) | many-to-many · `list` |

## UI Pages

| Page | URL | Enabled |
|------|-----|:-------:|
| List | `/en/creator` | ✓ |
| New | `/en/creator/new` | ✓ |
| Edit | `/en/creator/edit/[id]` | ✓ |
| View | `/en/creator/view/[id]` | ✓ |

## API Reference

### Authentication

All endpoints require one of:

| Header | Format | Example |
|--------|--------|---------|
| `X-API-Key` | `<key>` | `X-API-Key: mk_abc123...` |
| `Authorization` | `Bearer <key>` | `Authorization: Bearer mk_abc123...` |

API keys are stored in `user.api_key` and start with the `mk_` prefix.

---

### GET /api/creator

List all `Creator` records the authenticated user has permission to read.

**Response `200 OK`**

```json
[
  {
    "id": "clxxxxxxxxxxxxxxxxxxxxxxxx",
    "name": "...",
    "role": 0,
    "affiliation": 0
  }
]
```

### POST /api/creator

Create a new `Creator` record.

**Request Body**

```json
{
  "name": "...",
  "role": voice,
  "affiliation": agency,
  "voicedCharacters_ids": ["clxxx..."],
  "composedMusics_ids": ["clxxx..."],
  "creditedMusics_ids": ["clxxx..."],
  "creditedScenes_ids": ["clxxx..."]
}
```

**Child / association fields**

| Field | Type | Description |
|-------|------|-------------|
| `voicedCharacters_ids` | `string[]` | IDs of `character` records to associate (replaces existing) |
| `composedMusics_ids` | `string[]` | IDs of `music` records to associate (replaces existing) |
| `creditedMusics_ids` | `string[]` | IDs of `music` records to associate (replaces existing) |
| `creditedScenes_ids` | `string[]` | IDs of `scene` records to associate (replaces existing) |

**Response `201 Created`** — the newly created record object.

### GET /api/creator/[id]

Retrieve a single `Creator` record by its ID.

**Response `200 OK`** — the record object (same shape as a list item).

**Response `404 Not Found`**

```json
{ "error": "Not found" }
```

### PUT /api/creator/[id]

Replace an existing `Creator` record. 
Accepts the same body as `POST /api/creator`.

**Response `200 OK`**

```json
{ "success": true }
```

**Response `404 Not Found`**

```json
{ "error": "Not found" }
```

### DELETE /api/creator/[id]

Delete a `Creator` record by its ID.

**Response `204 No Content`** — empty body on success.

**Response `404 Not Found`**

```json
{ "error": "Not found" }
```


### Error Responses

| Status | Meaning |
|--------|---------|
| `400` | Bad request (validation error) |
| `401` | Missing or invalid API key |
| `403` | Insufficient permission for the operation |
| `404` | Record not found |
| `500` | Unexpected server error |

All error responses return `{ "error": "<message>" }`.

---

## Bulk Operations

Bulk endpoints process each item independently, enabling **partial success**. The outer
request always returns `207 Multi-Status` as long as authentication passes; individual
item outcomes are reported in the `results` array.

### Response shape (`207 Multi-Status`)

```json
{
  "results": [
    { "index": 0, "success": true,  "data": { ... } },
    { "index": 1, "success": false, "error": "Not found: clxxx..." }
  ],
  "summary": { "total": 2, "succeeded": 1, "failed": 1 }
}
```

> Auth/permission failures at the **request** level (bad key, no permission at all) still
> return `401` / `403` — not `207`.

### POST /api/creator/bulk

Bulk-create `Creator` records. Each element in the request array is processed with
the same rules as `POST /api/creator`.

**Request Body** — array of create objects:

```json
[
  {
    "name": "...",
    "role": voice,
    "affiliation": agency,
    "voicedCharacters_ids": ["clxxx..."],
    "composedMusics_ids": ["clxxx..."],
    "creditedMusics_ids": ["clxxx..."],
    "creditedScenes_ids": ["clxxx..."]
  },
  { "..." : "..." }
]
```

**Response `207`** — `data` of each successful item contains `{ "id": "..." }`.

### PUT /api/creator/bulk

Bulk-update `Creator` records. Each element must include `id` in addition to the
fields accepted by `PUT /api/creator/[id]`.

**Request Body** — array of update objects:

```json
[
  { "id": "clxxx...", 
    "name": "...",
    "role": voice,
    "affiliation": agency,
    "voicedCharacters_ids": ["clxxx..."],
    "composedMusics_ids": ["clxxx..."],
    "creditedMusics_ids": ["clxxx..."],
    "creditedScenes_ids": ["clxxx..."]
  },
  { "id": "clyyy...", "..." : "..." }
]
```

**Response `207`** — `data` of each successful item is `{ "success": true }`.
Items with an unknown `id` report `"error": "Not found: <id>"`.

### DELETE /api/creator/bulk

Bulk-delete `Creator` records by ID.

**Request Body** — array of id objects:

```json
[
  { "id": "clxxx..." },
  { "id": "clyyy..." }
]
```

**Response `207`** — `data` of each successful item is `null`.
Items with an unknown `id` report `"error": "Not found: <id>"`.

