...
| List | `/en/fc_link` | ✓ |
| New | `/en/fc_link/new` | ✗ |
| Edit | `/en/fc_link/edit/[id]` | ✓ |
| View | `/en/fc_link/view/[id]` | ✓ |
...
List all `Fc Link` records the authenticated user has permission to read.
...
Retrieve a single `Fc Link` record by its ID.
...
Replace an existing `Fc Link` record. 
...
### DELETE /api/fc_link/[id]

Delete a `Fc Link` record by its ID.
...
### PUT /api/fc_link/bulk

Bulk-update `Fc Link` records. Each element must include `id` in addition to the
fields accepted by `PUT /api/fc_link/[id]`.
...
### DELETE /api/fc_link/bulk

Bulk-delete `Fc Link` records by ID.
