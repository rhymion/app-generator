...
- **Fixed: composite/dotted `labelField` rendered blank (Issue #539)**: the read-only `valueGetter`'s fallback path (no `Config` passed — a many-to-one column inside an *embedded* child DataGrid, §7.1/§7.4) read the plain `row.reference?.name` shape even when `labelField` was composite or dotted. The child's Prisma `include` now resolves nested relations the same way the entity's own independent list page already did, so the composite/dotted label renders instead of coming back empty.
- **Fixed: `format: date`/`time` on the child's own field always showed a fixed date+time (Issue #540)**: a plain (non-relationship) `date`/`time`-formatted column inside the child grid ignored the field's declared `format` and always rendered a full timestamp. The column now reuses the shared `formatLabelValue()` formatter, the same one every other `date`/`time` column on this page goes through; a `date-time` column was unaffected (it was already using this formatter).
