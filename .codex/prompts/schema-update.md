# Schema Update - Type A

Treat this as a Type A (schema-only) task per `AGENTS.md`.
...
4. Run the Type A gate:
   ...
   - `npm run build`
   - `npm run cy:test:api`
5. If Python generators (`code_generator/*.py`) or non-generated TypeScript also
   need to change, stop and announce that this is now Type B, then switch to the
   Type B gate.
