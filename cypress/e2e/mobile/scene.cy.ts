...
//
// Mobile-viewport spec (Scene). The list page renders a different
...
describe('Testing Scene pages and their behavior [mobile]', () => {
  beforeEach(() => {
  ...
  describe('Card list', () => {
    ...
    it('1.2 renders one card per item with the primary value as title', () => {
      ...
      cy.visit('/en/scene');
      cy.contains('Test Label 1').should('be.visible');
  ...
  describe('Create from card list', () => {
    it('2.1 navigates to the new page via the toolbar add button', () => {
      ...
      // aria-label as the desktop variant ("Create New Scene").
      cy.get('button[aria-label="Create New Scene"]').click();
