...
describe('API: CSV Import batch2 (cmd_328) — dotted FK + Tier2/Tier3 authorization', () => {
  ...
  //
  // NOTE ON DESIGN DEVIATION (status code): subtask_328d's design assumed a 403
  // response. Empirical trace of lib/authz.ts + lib/api-auth.ts: assertPermission()
  // throws a plain `Error` (not `ApiError`); handleApiError() maps any non-ApiError
  // throw to 500, not 403 — the 403 path is only reached via requireApiPermission's
  // ApiError wrapper, which the main CRUD routes use (authenticateApiKey +
  // requireApiPermission) but the session-based import/export routes do not (they
  // call getModelPermissions + assertPermission directly). This is pre-existing
  // batch1 behavior — the export route template has the identical pattern — not
  // something introduced by batch2. Asserting the actual (500) behavior here;
  // flagged to gunshi/karo as a possible follow-up correctness fix (out of scope
  // for this task's declared file changes).
