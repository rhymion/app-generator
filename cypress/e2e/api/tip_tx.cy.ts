...
describe('API: Tip Tx', () => {
      ...
      cy.task<any>('db:populateTipTxDependencies').then((deps) => {
        cy.request({
          ...
          body: {
            ...
            operator_fee: 100,
            payment_fee: 100,
            contract_split_id: 'Test Contract Split Id',
            status: 0,
    ...
    it('4.1 updates, verified by GET', () => {
      cy.task<any[]>('db:populateTipTx', 1).then((records) => {
        cy.request({
          ...
          headers: { 'X-API-Key': TEST_API_KEY },
          body: {
            gross_amount: records[0].gross_amount,
            operator_fee: records[0].operator_fee,
            payment_fee: records[0].payment_fee,
            contract_split_id: records[0].contract_split_id,
            status: 1,
  ...
  describe('DELETE /api/tip_tx/bulk', () => {
  ...
  describe('Permission errors', () => {
    it('7.1 returns 4xx for GET list when permission denied', () => {
      cy.task<string>('db:createLimitedApiUser', 'tip_tx').then((limitedKey) => {
