...
describe('API: Creator', () => {
      ...
      cy.request({
        ...
        cy.request({ url: `${API_BASE}/${res.body.id}`, headers: { 'X-API-Key': TEST_API_KEY } })
          .then((getRes) => {
            ...
            expect(getRes.body.name).to.eq('Test Creator');
          });
      });
    });
      ...
      cy.task<any[]>('db:populateCreator', 1).then((records) => {
        cy.request({
          ...
          body: {
            ...
            role: records[0].role,
            affiliation: records[0].affiliation,
            voiced_characters: [],
            composed_musics: [],
            credited_musics: [],
            credited_scenes: [],
          ...
          cy.request({ url: `${API_BASE}/${records[0].id}`, headers: { 'X-API-Key': TEST_API_KEY } })
            .then((getRes) => {
              expect(getRes.status).to.eq(200);
              expect(getRes.body.name).to.eq('Updated Creator');
            });
        });
      });
    });
  ...
  describe('POST /api/creator/bulk', () => {
  ...
  describe('DELETE /api/creator/bulk', () => {
  ...
  describe('Permission errors', () => {
    it('7.1 returns 4xx for GET list when permission denied', () => {
      cy.task<string>('db:createLimitedApiUser', 'creator').then((limitedKey) => {
