...
// x-generate.test: true and api: true, generate.py's _write()
// (code_generator/generate.py) overwrites unconditionally — no exists()
// check — and will silently replace this file with generated content. No
// conflict error is raised; the only signal would be a diff after the next
// generate-code run.
import { TEST_API_KEY } from '../../support/test-credentials';

const API_BASE = '/api/audit_log';
...
describe('API: Audit Log', () => {
  ...
  describe('GET /api/audit_log/:id', () => {
    it('2.1 returns item detail by id', () => {
      cy.task<any[]>('db:populateAuditLog', 1).then((records) => {
