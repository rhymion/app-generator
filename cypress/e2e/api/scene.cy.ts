...
describe('API: Scene', () => {
        ...
        cy.request({
          ...
          cy.request({ url: `${API_BASE}/${res.body.id}`, headers: { 'X-API-Key': TEST_API_KEY } })
            .then((getRes) => {
              ...
              expect(getRes.body.label).to.eq('Test Label');
            });
        });
      });
    });
      ...
      cy.task<any[]>('db:populateScene', 1).then((records) => {
        cy.request({
          ...
          cy.request({ url: `${API_BASE}/${records[0].id}`, headers: { 'X-API-Key': TEST_API_KEY } })
            .then((getRes) => {
              expect(getRes.status).to.eq(200);
              expect(getRes.body.label).to.eq('Updated Label');
            });
        });
      });
    });
  ...
  describe('DELETE /api/scene/bulk', () => {
  ...
  describe('Permission errors', () => {
    it('7.1 returns 4xx for GET list when permission denied', () => {
      cy.task<string>('db:createLimitedApiUser', 'scene').then((limitedKey) => {
