    ...
    it('1.1 returns 1 seed-only item(s)', () => {
      cy.request({ url: API_BASE, headers: { 'X-API-Key': TEST_API_KEY } })
        .then((res) => {
          ...
          // db:seed + db:grantAllPermissions pre-populate 1 role record(s) before any user data
