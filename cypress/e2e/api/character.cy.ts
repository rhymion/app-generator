...
describe('API: Character', () => {
        ...
        cy.request({
          ...
          cy.request({ url: `${API_BASE}/${res.body.id}`, headers: { 'X-API-Key': TEST_API_KEY } })
            .then((getRes) => {
              ...
              expect(getRes.body.name).to.eq('Test Character');
            });
        });
      });
    });
      ...
      cy.task<any[]>('db:populateCharacter', 1).then((records) => {
        cy.request({
          ...
          body: {
            ...
            official_image: records[0].official_image,
            scenes: [],
            creators: [],
          },
        }).then((res) => {
  ...
  describe('POST /api/character/bulk', () => {
  ...
  describe('PUT /api/character/bulk', () => {
  ...
  describe('DELETE /api/character/bulk', () => {
  ...
  describe('Permission errors', () => {
    it('7.1 returns 4xx for GET list when permission denied', () => {
      cy.task<string>('db:createLimitedApiUser', 'character').then((limitedKey) => {
