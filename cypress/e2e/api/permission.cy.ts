...
describe('API: Permission', () => {
  ...
  describe('GET /api/permission', () => {
    it('1.1 returns 15 seed-only item(s)', () => {
      cy.request({ url: API_BASE, headers: { 'X-API-Key': TEST_API_KEY } })
        .then((res) => {
          expect(res.status).to.eq(200);
          // db:seed + db:grantAllPermissions pre-populate 15 permission record(s) before any user data
          expect(res.body.rows).to.have.length(15);
          expect(res.body.total).to.eq(15);
    ...
    it('1.2 returns page with items', () => {
      ...
      cy.request({ url: API_BASE, headers: { 'X-API-Key': TEST_API_KEY } })
        .then((res) => {
          expect(res.status).to.eq(200);
          expect(res.body.rows).to.have.length(16);
          expect(res.body.total).to.eq(16);
        });
    });
  });
