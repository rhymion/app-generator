...
describe('API: Dashboard', () => {
      ...
      cy.request({
        ...
        cy.request({ url: `${API_BASE}/${res.body.id}`, headers: { 'X-API-Key': TEST_API_KEY } })
          .then((getRes) => {
            ...
            expect(getRes.body.name).to.eq('Test Dashboard');
          });
      });
    });
  ...
  describe('POST /api/dashboard/bulk', () => {
  ...
  describe('PUT /api/dashboard/bulk', () => {
  ...
  describe('DELETE /api/dashboard/bulk', () => {
  ...
  describe('Permission errors', () => {
    it('7.1 returns 4xx for GET list when permission denied', () => {
      cy.task<string>('db:createLimitedApiUser', 'dashboard').then((limitedKey) => {
