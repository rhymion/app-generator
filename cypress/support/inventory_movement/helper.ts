...
export async function populateInventoryMovementDependencies() {
  ...
  const fromInventoryRecord = await prisma.inventory.create({
    data: {
      quantity: 0,
...
export async function populateInventoryMovementData(length: number) {
  const testUser = await getTestUser();
  const deps = await populateInventoryMovementDependencies();
  const records = [];
  for (let i = 1; i <= length; i++) {
    const approvableItem = await prisma.approvable.create({ data: {} });
    const record = await prisma.inventory_movement.create({
      data: {
        approvable_id: approvableItem.id,
        from_inventory_id: deps.fromInventory.id,
...
export async function populateInventoryMovementFullData(length: number) {
  const testUser = await getTestUser();
  const deps = await populateInventoryMovementDependencies();
  const records = [];
  for (let i = 1; i <= length; i++) {
...
export async function setupInventoryMovementApprovalFlow() {
  ...
  const requestorRole = await prisma.role.create({
    data: { name: 'Test To Inventory Requestor Role', creator_id: testUser.id, updater_id: testUser.id },
  });
  const approverRole = await prisma.role.create({
    data: { name: 'Test To Inventory Approver Role', creator_id: testUser.id, updater_id: testUser.id },
  });
  // noRoleUser needs entity permission to access inventory_movement but must NOT hold
  // requestorRole/approverRole (so the role-gated approval flow is skipped for them).
  const basicRole = await prisma.role.create({
    data: { name: 'Test To Inventory Basic Role', creator_id: testUser.id, updater_id: testUser.id },
  });
  ...
  const requestorUser = await prisma.user.create({
    data: {
      name: 'Test To Inventory Requestor User',
      email: 'test-inventory_movement-requestor@example.com',
      password: hashedPw,
      api_key: `test_mk_inventory_movement_requestor`,
      creator_id: testUser.id,
      updater_id: testUser.id,
  ...
  const approverUser = await prisma.user.create({
    data: {
      name: 'Test To Inventory Approver User',
      email: 'test-inventory_movement-approver@example.com',
      password: hashedPw,
      api_key: `test_mk_inventory_movement_approver`,
      creator_id: testUser.id,
      updater_id: testUser.id,
  ...
  const noRoleUser = await prisma.user.create({
    data: {
      name: 'Test To Inventory No Role User',
      email: 'test-inventory_movement-norole@example.com',
      password: hashedPw,
      api_key: `test_mk_inventory_movement_norole`,
      creator_id: testUser.id,
      updater_id: testUser.id,
  ...
  const flowWithRole = await prisma.approval_flow.create({
    data: {
      entity_name: 'inventory_movement',
...
export async function setupInventoryMovementOrderedApprovalFlow() {
  const { hashPassword } = require('../test-credentials');
  const testUser = await getTestUser();
  const hashedPw = await hashPassword('test-password');

  const approverRole1 = await prisma.role.create({
    data: { name: 'Test To Inventory Ordered Approver Role 1', creator_id: testUser.id, updater_id: testUser.id },
  });
  const approverRole2 = await prisma.role.create({
    data: { name: 'Test To Inventory Ordered Approver Role 2', creator_id: testUser.id, updater_id: testUser.id },
  });
  ...
  const approverUser1 = await prisma.user.create({
    data: {
      name: 'Test To Inventory Ordered Approver 1',
      email: 'test-inventory_movement-ordered-approver1@example.com',
      password: hashedPw,
      api_key: `test_mk_inventory_movement_ordered_approver1`,
      creator_id: testUser.id,
      updater_id: testUser.id,
  ...
  const approverUser2 = await prisma.user.create({
    data: {
      name: 'Test To Inventory Ordered Approver 2',
      email: 'test-inventory_movement-ordered-approver2@example.com',
      password: hashedPw,
      api_key: `test_mk_inventory_movement_ordered_approver2`,
      creator_id: testUser.id,
      updater_id: testUser.id,
...
export async function populateInventoryMovementWithRejectedApproval(creatorId: string, approvalFlowIds: string[]) {
  const testUser = await getTestUser();
  const deps = await populateInventoryMovementDependencies();
  const approvableItem = await prisma.approvable.create({ data: {} });
  const record = await prisma.inventory_movement.create({
...
export async function populateInventoryMovementWithTerminalRejectedApproval(creatorId: string, approvalFlowIds: string[]) {
  const testUser = await getTestUser();
  const deps = await populateInventoryMovementDependencies();
  const approvableItem = await prisma.approvable.create({ data: {} });
  const record = await prisma.inventory_movement.create({
...
export async function populateInventoryMovementWithApproval(creatorId: string, approvalFlowIds: string[]) {
  const testUser = await getTestUser();
  const deps = await populateInventoryMovementDependencies();
  const approvableItem = await prisma.approvable.create({ data: {} });
  const record = await prisma.inventory_movement.create({
