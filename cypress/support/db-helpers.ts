...
export async function resetTestDatabase() {
  ...
  // Level 2: approval_edit_terminal_test, approval_request, attachment, dashboard_widget, notification, organization, permission, reaction
  await prisma.approval_edit_terminal_test.deleteMany();
 ...
 * Needed for cmd_452 org-isolation X-API-Key route tests (detail GET/PUT/DELETE,
 * list, export): createLimitedApiUser only covers the all-false explicit-deny
 * case, and createSessionUserWithPermission issues no api_key (session-only).
 *
 * `organizationId`, when supplied, enrolls the new user in that one organization
 * — org membership (not `organization`-entity read permission) is what governs
 * reaching a should_filter_by_org row the actor belongs to, so a fixture
 * exercising "has the RBAC role but can't read the `organization` entity itself"
 * must still grant membership, or every should_filter_by_org PUT/DELETE
 * existence check (`api_detail_route.ts.jinja2`, `getAssociatedOrganizations`-
 * scoped) 404s before the scenario under test is ever reached — org isolation
 * firing, not the thing being tested (cmd_799). Omit it (or pass no
 * organization at all) for the original cmd_452 IDOR fixture shape: an actor
 * enrolled in zero organizations, deliberately a different org from any row's.
 */
export async function createApiUserWithPermission(
  ...
  await prisma.user.create({
    data: {
      ...
      ...(organizationId ? { organizations: { connect: [{ id: organizationId }] } } : {}),
    },
  });
