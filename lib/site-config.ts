...
export const siteConfig = {
  /**
   * Title shown in the browser tab and the header bar. NEXT_PUBLIC_APP_TITLE
   * is baked in at build time (Next.js inlines NEXT_PUBLIC_ vars into the
   * client bundle) — changing it on Vercel requires a rebuild, not just a
   * redeploy.
   */
  ...
  navLinks: [
    ...
    { label: "Approval Edit Terminal Test", href: "/approval_edit_terminal_test" },
  ] satisfies NavLink[],
  ...
  /**
   * Search-engine indexing control. Generated apps are primarily internal
   * tools, so indexing is blocked by default — this holds even if this whole
   * `seo` block, or just `noindex` within it, is removed (see app/layout.tsx,
   * which treats a missing value as `true`). Set `noindex: false` here to
   * allow indexing (e.g. a public-facing deployment).
   */
  seo: {
    noindex: true,
  } as { noindex?: boolean },
  ...
  /**
   * Text shown in the site footer. NEXT_PUBLIC_APP_COPYRIGHT is baked in at
   * build time, same as NEXT_PUBLIC_APP_TITLE above — a Vercel rebuild is
   * required to pick up a changed value.
   */
