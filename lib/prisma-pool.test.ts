...
describe('resolvePrismaPoolMax', () => {
  it('falls back to the default when unset', () => {
    expect(resolvePrismaPoolMax(undefined)).toBe(2);
  });

  it('falls back to the default when the empty string', () => {
    expect(resolvePrismaPoolMax('')).toBe(2);
  });
  ...
  it('parses a valid override with a non-default fallback', () => {
    expect(resolvePrismaPoolMax(undefined, 5)).toBe(5);
  });

  it('falls back to the default for a non-numeric value', () => {
    expect(resolvePrismaPoolMax('not-a-number')).toBe(2);
  });

  it('falls back to the default for zero', () => {
    expect(resolvePrismaPoolMax('0')).toBe(2);
  });

  it('falls back to the default for a negative value', () => {
    expect(resolvePrismaPoolMax('-5')).toBe(2);
  });
