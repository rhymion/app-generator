...
- **整数型 enum** — `x-enum` フィールドに整数値を指定すると、正しい TypeScript 判別子を持つ整数型 Prisma enum を生成
- **ラッパーコンポーネントアーキテクチャ** — エンティティごとに生成されたコンポーネントが `components/_standard/` の共有ラッパーを使用（静的提供；`generate-code` 再実行で上書きされない）
...
| 整数型 enum（x-enum） | ✅ 実装済み |
| ラッパーコンポーネントアーキテクチャ | ✅ 実装済み |

> **後方互換（v1.2 → v1.3）**: 非破壊的変更。既存のスキーマはそのまま動作します。`x-reservation`・整数型 `x-enum`・`x-display.dashboard` はオプトインです。新機能を使用しない限り、既存プロジェクトへの対応は不要です。
