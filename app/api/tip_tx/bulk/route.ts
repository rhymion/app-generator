...
// ---------------------------------------------------------------------------
// POST /api/tip_tx/bulk  — bulk create
// ---------------------------------------------------------------------------
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < bulkItems.length; i++) {
      try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const { gross_amount: grossAmount, operator_fee: operatorFee, payment_fee: paymentFee, contract_split_id: contractSplitId, status, comment_id: commentId } = bulkItems[i] as any;
...
// ---------------------------------------------------------------------------
// PUT /api/tip_tx/bulk  — bulk update
// ---------------------------------------------------------------------------
export async function PUT(request: NextRequest) {
  try {
    const { userId: actorId } = await authenticateApiKey(request);
    const richPerms = await requireApiPermission(actorId, 'tip_tx', 'update');
    const body = await request.json();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const bulkItems: any[] = Array.isArray(body) ? body : [body];
    ...
    const existingRecords = await prisma.tip_tx.findMany({
    ...
    for (let i = 0; i < bulkItems.length; i++) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { id, gross_amount: grossAmount, operator_fee: operatorFee, payment_fee: paymentFee, contract_split_id: contractSplitId, status, comment_id: commentId } = bulkItems[i] as any;
...
// ---------------------------------------------------------------------------
// DELETE /api/tip_tx/bulk  — bulk delete
// ---------------------------------------------------------------------------
export async function DELETE(request: NextRequest) {
  try {
    const { userId: actorId } = await authenticateApiKey(request);
    const richPerms = await requireApiPermission(actorId, 'tip_tx', 'delete');
    const body = await request.json();
    ...
    if (permitted.length > 0) {
      await deleteTipTx(permitted.map((p) => p.id));
