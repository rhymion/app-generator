...
export async function PUT(request: NextRequest) {
  try {
    const { userId: actorId } = await authenticateApiKey(request);
    const richPerms = await requireApiPermission(actorId, 'role', 'update');
    const body = await request.json();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const bulkItems: any[] = Array.isArray(body) ? body : [body];
    ...
    for (let i = 0; i < bulkItems.length; i++) {
      ...
      try {
        await updateRole(actorId, id, name, description ?? null, users_ids ?? [], null);
...
export async function DELETE(request: NextRequest) {
  try {
    const { userId: actorId } = await authenticateApiKey(request);
    const richPerms = await requireApiPermission(actorId, 'role', 'delete');
    const body = await request.json();
    ...
    if (permitted.length > 0) {
      await deleteRole(actorId, permitted.map((p) => p.id));
