...
import { updateCreator, deleteCreator } from '@/lib/creator/service';
...
// ---------------------------------------------------------------------------
// POST /api/creator/bulk  — bulk create
// ---------------------------------------------------------------------------
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < bulkItems.length; i++) {
      try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const { name, role, affiliation, voicedCharacters_ids, composedMusics_ids, creditedMusics_ids, creditedScenes_ids } = bulkItems[i] as any;
...
// ---------------------------------------------------------------------------
// PUT /api/creator/bulk  — bulk update
// ---------------------------------------------------------------------------
export async function PUT(request: NextRequest) {
  try {
    const { userId: actorId } = await authenticateApiKey(request);
    const richPerms = await requireApiPermission(actorId, 'creator', 'update');
    const body = await request.json();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const bulkItems: any[] = Array.isArray(body) ? body : [body];
    ...
    const existingRecords = await prisma.creator.findMany({
    ...
    for (let i = 0; i < bulkItems.length; i++) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { id, name, role, affiliation, voicedCharacters_ids, composedMusics_ids, creditedMusics_ids, creditedScenes_ids } = bulkItems[i] as any;
      ...
      try {
        await updateCreator(actorId, id, name, role, affiliation, voicedCharacters_ids ?? [], composedMusics_ids ?? [], creditedMusics_ids ?? [], creditedScenes_ids ?? [], null);
...
// ---------------------------------------------------------------------------
// DELETE /api/creator/bulk  — bulk delete
// ---------------------------------------------------------------------------
export async function DELETE(request: NextRequest) {
  try {
    const { userId: actorId } = await authenticateApiKey(request);
    const richPerms = await requireApiPermission(actorId, 'creator', 'delete');
    const body = await request.json();
    ...
    if (permitted.length > 0) {
      await deleteCreator(permitted.map((p) => p.id));
