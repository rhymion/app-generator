...
export async function PUT(request: NextRequest) {
  try {
    ...
    const richPerms = await requireApiPermission(actorId, 'setting', 'update');
    ...
    for (let i = 0; i < bulkItems.length; i++) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { id, name, email, password, api_key: apiKey, image, roles_ids } = bulkItems[i] as any;
      ...
      try {
        await updateSetting(actorId, id, name, email, password ?? null, apiKey ?? null, image ?? null, roles_ids ?? [], null);
