...
async function UserViewContent({ id }: { id: string }) {
  const [{ user, userPermissions }, currentUserRoleIds, currentUserId] = await Promise.all([
    getUserDetailPageData(id),
    getUserRoleIds(),
    getSessionUserId(),
  ]);
  if (!user) {
  ...
  return <FormView src={user} permissions={userPermissions} currentUserRoleIds={currentUserRoleIds} currentUserId={currentUserId} />;
}
